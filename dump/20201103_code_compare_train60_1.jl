...
Compare the 60 sec training protocol, with both E and I plasticity
and heterosynaptic normalization.
The latter is implemented by "brute force" exactly in the same 
way in both models (for simplicity)
=#
    ...
    for cc = 1:Ne
      ...
      for dd = 1:Ne
        if weights[dd,cc] > 0.
          ...
          weights[dd,cc] = hardbounds(weights[dd,cc],wmin,wmax)
        end # if
      end # dd for
    end # cc for
...
using StatsBase
function do_psth_au(vals::Vector{<:Real},times_psth::AbstractVector{<:Real};dt=0.1E-3)
  times = dt/2:dt:Ttot-dt/2 
  ret = fill(0.0,length(times_psth)-1)
  for (v,tt) in zip(vals,times)
    i = searchsortedfirst(times_psth,tt)-1
    ret[i] += v
  end
  dt_psth = times_psth[2] - times_psth[1]
  return ret ./ dt_psth
end
...
times_psth = let dtstep = dt_psth
  range(0,Ttot;step=dtstep)
end
